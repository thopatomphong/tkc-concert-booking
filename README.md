...
The host handles networking (providing an authenticated `Dio`) and navigation back to the Core App.

```dart
class MyConcertHost implements ConcertHost {
  @override
  Dio get httpClient => MyAuthService.getAuthenticatedDio();
  ...
  @override
  void onExit() {
    // Navigate away from the Mini App
...
- `httpClient`: Must return a `Dio` instance. The Mini App uses this for all API requests.
